...
const limiter = new Bottleneck({
  maxConcurrent: parseInt(settings.STREAM_DOWNLOAD_CONCURRENCY_LIMIT || 5),
  minTime: 333,
});
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const subscription = streamClient.subscribe(channel, (data) => {
      ...
      if (!data?.event?.replayId) {
        reject(prefix + "missing replayId on data object");
...
async function main(prefix = "salesforce/") {
  ...
  for await (const orgPrefix of storage.list(prefix)) {
    ...
    for await (const userPrefix of storage.list(orgPrefix)) {
      ...
      promises.push(
        limiter.schedule(() => stream(userPrefix).catch(console.error))
      );
    }
  }
...
if (require.main === module) {
  main()
    .then((res) => {
      console.log("all streams done", res);
      process.exit();
    })
    .catch(console.error);
}
